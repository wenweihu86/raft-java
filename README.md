# raft-java
this project forks [wenweihu86/raft-java](https://github.com/wenweihu86/raft-java).<br>
修复了原project serviceInterface must not be set repeatedly, please use another RpcClient的issue
...
## Quick Start
1. 在本地单机上部署一套3实例的raft集群，执行如下脚本：<br>
cd raft-java-example && sh deploy.sh <br>
2. 该脚本会在raft-java-example/env目录部署三个实例example1、example2、example3；<br>
同时会创建一个client目录，用于测试raft集群读写功能。<br>
3. 部署成功后，测试写操作，通过如下脚本：
cd env/client <br>
./bin/run_client.sh "list://127.0.0.1:8051,127.0.0.1:8052,127.0.0.1:8053" hello world <br>
出现 set request, key=hello value=world response={"success": true}，表示写操作测试成功 <br>
4. 测试读操作命令：<br>
./bin/run_client.sh "list://127.0.0.1:8051,127.0.0.1:8052,127.0.0.1:8053" hello
出现 get request, key=hello, response={"value": "world"}，表示读操作测试成功
